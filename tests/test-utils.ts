...
export const mockForkSandboxSuccess = (
  ...
  // Only add private_preview if explicitly provided
  if (options?.private_preview !== undefined) {
    requestBody.private_preview = options.private_preview;
  }

  return nock("https://api.codesandbox.io")
    .post("/sandbox/pcz35m/fork", requestBody)
    .reply(200, {
      data: {
        id: sandboxId,
...
/**
 * Properly cleanup test sandbox with correct sequencing:
 * 1. Wait for client cleanup (disconnect and dispose)
 * 2. Wait for shutdown with 10 second timeout
 * 3. Fire-and-forget delete (with small delay to ensure request goes through)
 */
export const cleanupTestSandbox = async (
